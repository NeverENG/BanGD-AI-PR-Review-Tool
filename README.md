...
- **响应快、成本低。** 大块固定提示词（人设 + rubric + 范例）启用 prompt caching，每个 PR 只有 diff 这条尾巴是未缓存的。
...
permissions:
  ...
  pull-requests: write    # 发表评审评论（缺这条会发不出评论）
...
3. 跑完后，BanGD 会在 PR 上贴一条评论：顶部是风险徽章 + 变更总结，下面是逐条 finding。
